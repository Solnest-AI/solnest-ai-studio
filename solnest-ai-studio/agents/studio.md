...
2. **PLAN** — write a short plan **plus an explicit DEFINITION OF DONE** (acceptance criteria, which tests must pass, what "good" looks like). → ⏸ pause for plan approval only when the plan contains a costly-to-reverse choice (API shape, DB schema, data migration, dependency); otherwise post the plan + DoD and proceed.
